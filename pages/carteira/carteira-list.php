...
$condicoes = [
    strlen($buscar) ? 'e.nome LIKE "%' . str_replace(' ', '%', $buscar) . '%"' : null
];
...
$qtd = Carteira::getQtd($where);

$pagination = new Pagination($qtd, $_GET['pagina'] ?? 1, 100);

$listar = Carteira::getList(' e.id AS id, e.tipo AS tipo, e.forma_pagamento_id AS forma_pagamento_id, e.apelido AS apelido,
                              ...
                              'entregadores AS e INNER JOIN veiculos AS v ON (e.veiculos_id = v.id)', $where, 'e.apelido ASC', $pagination->getLimit());
